...
This compact diagram keeps only the main runtime blocks used by Fusion Flight. It is derived from the implemented flow in `scr_game_handle()`, `game_init()`, `game_update()`, `game_spawn_vehicle()`, `vehicle_try_change_lane()`, `game_check_collision()` and `view_scr_game()`.

```mermaid
flowchart LR
    Player["Player Input"]
    Buttons["Button Driver<br/>UP / DOWN / MODE"]
    AK["AK Kernel<br/>Message + Timer"]
    Screen["scr_game_handle()"]
    Init["game_init()<br/>Reset player, score, level,<br/>vehicle pool"]
    Update["game_update()<br/>Difficulty, jump,<br/>spawn, movement, score"]
    Pool["Vehicle Pool[7]<br/>Moto / F1 / Container"]
    Lane["Lane Change<br/>vehicle_try_change_lane()"]
    Collision["Collision + Jump Check<br/>game_check_collision()<br/>jump_has_clear_speed()"]
    Render["view_scr_game()<br/>HUD, lanes, sprites,<br/>Game Over overlay"]
    OLED["OLED Framebuffer<br/>128 x 64, 1-bit"]

    Player --> Buttons --> AK --> Screen
    Screen -->|SCREEN_ENTRY| Init
    Screen -->|AC_DISPLAY_GAME_TICK<br/>80 ms| Update
...
- `SCREEN_ENTRY` calls `game_init()` and starts the periodic `AC_DISPLAY_GAME_TICK` timer.
- Every `80 ms`, `game_update()` updates difficulty, jump/return movement, vehicle spawn, vehicle movement and scoring.
- Vehicles live in a fixed `vehicles[MAX_VEHICLES]` pool, currently `7` slots.
- Moto and F1 can change lane from level 2 onward. Container does not change lane.
- Collision uses per-object hitboxes, not full bitmap width.
- Jumping avoids a crash when the vehicle is jumpable or `jump_has_clear_speed()` allows the player to clear a larger object.
- `view_scr_game()` is render-only: it draws HUD, lanes, bitmaps and the Game Over overlay to the OLED framebuffer.
